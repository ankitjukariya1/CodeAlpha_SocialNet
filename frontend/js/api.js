...
async function apiRequest(path, method = "GET", body = null, auth = true) {
  ...
  try {
    ...
    throw {
      message:
        "Network error: Could not reach the server. Make sure the backend is running on " +
        API_BASE,
    };
  }
  if (!res.ok) {
...
export async function createPostWithImage(formData) {
  ...
  try {
    ...
    throw { message: "Network error: Could not reach the server." };
  }
  if (!res.ok) {
    const errorData = await res
      .json()
      .catch(() => ({ message: "Failed to create post" }));
    throw { message: errorData.message || "Failed to create post" };
  }
  return res.json();
}
export async function editPost(postId, formData) {
  ...
  let res;
  try {
    res = await fetch(`${API_BASE}/posts/${postId}`, {
      method: "PUT",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      body: formData,
    });
  ...
  if (!res.ok) {
    const errorData = await res
      .json()
      .catch(() => ({ message: "Failed to edit post" }));
    throw { message: errorData.message || "Failed to edit post" };
  }
  return res.json();
}
...
export async function uploadAvatar(formData) {
  ...
  if (!res.ok) {
    ...
    throw { message: errorData.message || "Failed to upload avatar" };
  }
  return res.json();
}
export async function getAllTags() {
  ...
  if (!res.ok) {
    const errorData = await res
      .json()
      .catch(() => ({ message: "Failed to load tags" }));
    throw { message: errorData.message || "Failed to load tags" };
  }
  return res.json();
}
